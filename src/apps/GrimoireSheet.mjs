...
export default class GrimoireSheet extends BaseWFRP4eItemSheet {
  ...
  async _onRender(context, options) {
    ...
    this.element.querySelectorAll(".spell img").forEach(e => e.addEventListener(
      "click",
      this.#onSpellClick.bind(this),
    ));
    this.element.querySelectorAll(".spells-remove").forEach(e => e.addEventListener(
      "click",
      this.#onSpellsRemoveClick.bind(this),
    ));
    this.element.querySelectorAll(".generate-description").forEach(e => e.addEventListener(
      "click",
      this.#onGenerateDescriptionClick.bind(this),
    ));
    this.element.querySelectorAll(".spell-name input").forEach(e => e.addEventListener(
      "change",
      this.#onSpellLabelChange.bind(this),
    ));
  }
