...
export default class TemporaryRunes extends ForienBaseModule {
  ...
  async damageWeapon(item, actor) {
    ...
    let regex = /\d{1,3}/gm;
    let maxDamage = Number(regex.exec(item.damage.value)[0] || 0) + Number(item.properties.qualities.durable?.value
                    || 0) || 999;
  ...
  async damageArmour(item, actor) {
    ...
    armourToDamage.system.APdamage[location] =
      Math.min(
        ...
        armourToDamage.system.APdamage[location] + 1,
      );

    let locationName = game.i18n.localize(`WFRP4E.Locations.${location}`);
    itemDamaged =
      `${game.i18n.format("Forien.Armoury.Runes.ArmourReceived1DamageOnLocation", {locationName: locationName})}.`;
